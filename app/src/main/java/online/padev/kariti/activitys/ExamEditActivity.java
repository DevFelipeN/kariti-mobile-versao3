...
public class ExamEditActivity extends AppCompatActivity {
        ...
        btnNext.setOnClickListener(v -> {
            ...
            try {
                ...
                if (examCurrent.getNameExam().trim().isEmpty()) { //verifica se o campo prova esta vazio
                    Toast.makeText(ExamEditActivity.this, "Informe o nome da Prova!", Toast.LENGTH_SHORT).show();
                    return;
                }
                ...
                if (examCurrent.isDifferent(examBD)) { //Verifica se os dados da prova foram alterados
                    if (!examCurrent.getNameExam().equals(examBD.getNameExam()) || !examCurrent.getClass_id().equals(examBD.getClass_id())) {
                        ...
                        if (verificaProva == null) {
                            Toast.makeText(ExamEditActivity.this, "Erro na comunicação, tente novamente!", Toast.LENGTH_SHORT).show();
                            return;
                        }
                        if (verificaProva) {
                            Toast.makeText(ExamEditActivity.this, "Esta turma já pussui uma prova cadastrada com esse nome, " + examCurrent.getNameExam(), Toast.LENGTH_SHORT).show();
                            return;
                        }
                    }
                ...
                Toast.makeText(ExamEditActivity.this, "Algo de errado ocorreu, tente novamente!", Toast.LENGTH_SHORT).show();
    ...
    private void carregarTelaGabarito(){
        ...
        Intent intent = new Intent(getApplicationContext(), AnswerKeyActivity.class);
        if (status == 0){
